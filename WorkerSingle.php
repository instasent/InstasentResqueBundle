...
class WorkerSingle extends \Resque_Worker
{
    ...
    public function work($interval = \Resque::DEFAULT_INTERVAL, $blocking = false)
    {
        ...
        while (true) {
            ...
            if (!$this->paused) {
                if ($blocking === true) {
                    $this->logger->log(LogLevel::INFO, 'Starting blocking with timeout of {interval}', ['interval' => $interval]);
    ...
    public function perform(\Resque_Job $job)
    {
        try {
            ...
            $this->logger->log(LogLevel::CRITICAL, '{job} has failed {stack}', ['job' => $job, 'stack' => $exception]);
            $job->fail($exception);
