...
class InstasentResqueExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (!empty($config['prefix'])) {
            ...
            $container->getDefinition('instasent_resque.resque')->addMethodCall('setPrefix', [$config['prefix']]);
        }
        ...
        if (!empty($config['auto_retry'])) {
            if (isset($config['auto_retry'][0])) {
                $container->getDefinition('instasent_resque.resque')->addMethodCall('setGlobalRetryStrategy', [$config['auto_retry'][0]]);
            } else {
                if (isset($config['auto_retry']['default'])) {
                    $container->getDefinition('instasent_resque.resque')->addMethodCall('setGlobalRetryStrategy', [$config['auto_retry']['default']]);
                    unset($config['auto_retry']['default']);
                }
                $container->getDefinition('instasent_resque.resque')->addMethodCall('setJobRetryStrategy', [$config['auto_retry']]);
            }
        }
    }
}
